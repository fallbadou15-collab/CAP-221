...
window.generateLetter = async function () {
    ...
    output.innerHTML = '<i class="fas fa-spinner fa-spin" style="color:var(--primary);"></i> ABF Assistant prépare ton CV et ta lettre... Cela prend quelques secondes.';
    ...
    try {
        ...
        if (err.code === 'AI_NOT_CONFIGURED') output.textContent = "ABF Assistant n'est pas encore configuré sur ce serveur.";
...
async function getAIReply(message) {
    try {
        const promptSysteme = "Tu es ABF Assistant, l'assistant virtuel de 'CAP 221', la meilleure plateforme d'orientation au Sénégal créée par Alioune Badara Fall. Réponds de manière courte, gentille et très précise à cette question d'un élève sénégalais : " + message;
        return await requestAI(promptSysteme);
    } catch (error) {
        if (error.code === 'AI_NOT_CONFIGURED') return "ABF Assistant n'est pas encore configuré sur ce serveur.";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sendChat) {
        sendChat.onclick = async () => {
            ...
            td.className = 'message bot'; td.id = tid; td.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ABF Assistant cherche…';
...
const dailyTips = [
    ...
    "Crée un premier CV avec ABF Assistant, même imparfait : on corrige plus vite ce qui existe déjà."
];
